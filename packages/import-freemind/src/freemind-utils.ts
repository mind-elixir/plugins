...
import type { FreeMindMap, FreeMindNode, FreeMindFont, FreeMindEdge } from './types/freemind-model'
...
function convertFreeMindNode(freemindNode: FreeMindNode): NodeObj {
  ...
  if (freemindNode.node && freemindNode.node.length > 0) {
    ...
    mindElixirNode.children = freemindNode.node.map(childNode => convertFreeMindNode(childNode))
