...
import type { Sheet, Topic, Style, Theme as XMindTheme } from './types/xmind-model'
import MindElixir, { type MindElixirData, type NodeObj, type Theme as MindElixirTheme } from 'mind-elixir'
...
function convertStyle(xmindStyle: Style): NodeObj['style'] {
  ...
  // 背景颜色 - XMind可能使用多种属性名
  const backgroundColor = properties['svg:fill'] || properties['background-color'] || properties['fill']
  if (backgroundColor) {
  ...
  const borderColor = properties['border-line-color'] || properties['svg:stroke'] || '#000000'
...
export function convertTheme(xmindTheme: XMindTheme): MindElixirTheme {
  ...
  const theme: MindElixirTheme = {
    ...
    palette: ['#3498db', '#e74c3c', '#2ecc71', '#f39c12', '#9b59b6', '#1abc9c', '#34495e', '#e67e22'], // 默认调色板
...
function convertXMLToJson(xmlText: string): Sheet[] {
  ...
  const sheets = Array.isArray(xmapContent.sheet) ? xmapContent.sheet : [xmapContent.sheet]
    ...
    sheet.relationships = relationships.map((rel: any) => ({
      ...
      controlPoints:
        rel['control-points'] && rel['control-points']['control-point']
          ? {
              '0': {
                x: rel['control-points']['control-point'][0]?.position?.['svg:x'] || 0,
                y: rel['control-points']['control-point'][0]?.position?.['svg:y'] || 0,
              },
              '1': {
                x: rel['control-points']['control-point'][1]?.position?.['svg:x'] || 0,
                y: rel['control-points']['control-point'][1]?.position?.['svg:y'] || 0,
              },
            }
          : undefined,
    }))
...
function convertXMLTopicToJSON(xmlTopic: any): Topic {
    ...
    if (Array.isArray(xmlTopic.children.topics)) {
      ...
      xmlTopic.children.topics.forEach((topicsGroup: any) => {
        ...
        if (topicsGroup.topic) {
          const topics = Array.isArray(topicsGroup.topic) ? topicsGroup.topic : [topicsGroup.topic]
  ...
  if (xmlTopic.summaries && xmlTopic.summaries.summary) {
    const summaries = Array.isArray(xmlTopic.summaries.summary) ? xmlTopic.summaries.summary : [xmlTopic.summaries.summary]
...
function convertTopic(summaries: MindElixirData['summaries'], xmindTopic: Topic, meTopic: NodeObj) {
  meTopic.id = xmindTopic.id || Math.random().toString(36).substring(2, 11)
  meTopic.topic = xmindTopic.title
  if (xmindTopic.notes?.plain.content) meTopic.note = xmindTopic.notes?.plain.content
    ...
    for (let i = 0; i < xmindTopic.summaries.length; i++) {
      ...
      xmindTopic.children?.['summary'].find(summaryTopic => {
        if (summaryTopic.id === summary.topicId) {
          ...
          const end = parseInt(summary.range.split(',')[1].substring(0, summary.range.split(',')[1].length - 1))
  ...
  if (data.relationships) {
    mindElixirData.arrows = data.relationships.map(relationship => {
      return {
        ...
        delta1: relationship.controlPoints ? relationship.controlPoints['0'] : { x: 50, y: 50 },
        delta2: relationship.controlPoints ? relationship.controlPoints['1'] : { x: 50, y: 50 },
      }
    })
  }
